...
BOOL CDIBSectionLite::SetBitmap(LPBITMAPINFO lpBitmapInfo, LPVOID lpBits/* = NULL*/)
{
    DeleteObject();
...
BOOL CDIBSectionLite::CreatePalette()
{
    ...
    RGBQUAD *pRGB = (RGBQUAD *)malloc(m_iColorTableSize * sizeof(RGBQUAD));
    if (!pRGB)
    {
        ReleaseDC(NULL, hDC);
											 ...
											 int iHeight)
{
	...
	for (int c = 0; c < iHeight; c++)
	{
		...
		{
			pLine[(c*iWidth) + r] = FixColorRef(tempDC.GetPixel(r, c));
		}
	}
	*/
	// Copy the bits into our 32 bit dib..
	for (int c = 0; c < iHeight; c++)
	{
		for (int r = 0; r < iWidth; r++)
		{
			pLine[((iHeight - 1 - c) * iWidth) + r] = FixColorRef(tempDC.GetPixel(r, c));
		}
	}
